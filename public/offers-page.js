...
async function loadOffers() {
    try {
        console.log('🎯 Loading offers and products...');
        
        // Load all products
        const productsResponse = await fetch('http://localhost:3002/api/products');
        offersProducts = await productsResponse.json();
...
async function saveOffer() {
    ...
    try {
        const response = await fetch(`http://localhost:3002/api/products/${currentOfferProduct.id}/offer`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ offer_percentage: percentage })
        });
...
async function updateProductOffer(productId, offerPercentage) {
    try {
        const response = await fetch(`http://localhost:3002/api/products/${productId}/offer`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ offer_percentage: offerPercentage })
        });
